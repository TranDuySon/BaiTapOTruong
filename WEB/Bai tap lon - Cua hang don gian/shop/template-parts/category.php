<?php 
	if( isset(  $_GET["id"])  && (is_numeric($_GET["id"])) ){
		...
		$sql = "SELECT * FROM hoa WHERE menu={$_GET['id']} ";
		$rs = mysqli_query($conn, $sql);
		if( mysqli_num_rows($rs)  <= 0 ){
			echo alert("Danh mục không tồn tại :'( " , "alert-error");
			return;
		}
	}
...
?>
	...
	<div class="block-body">
		<div class="block-body-title">
			<h2>Các sản phẩm trong đanh mục : <?php  echo get_menu_name_by_id($_GET["id"]); ?></h2>
		</div>
